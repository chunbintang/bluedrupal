...
class OpenStack extends Client
{
    ...
    public function __construct($url, array $secret, array $options = array())
    {
		...
		$newurl=$url;
		if(!empty($secret['username']))
		{
		    //Add the /username to base url as requested by Bluemix
			$newurl=Url::factory($url)->addPath($secret['username']);
		}
		
        $this->setAuthUrl($newurl);

        parent::__construct($newurl);
    ...
    public function authenticate()
    {
		...
		$authField= 'Basic '.base64_encode($this->secret['username'].':'.$this->secret['password']);
		$authHeader = array(
     ...
     * @return void
     */
    public function updateTokenHeader($token = null)
    {
